<?php
	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'visita' => "`visita`.`id` as 'id', if(`visita`.`fecha`,date_format(`visita`.`fecha`,'%m/%d/%Y'),'') as 'fecha', `visita`.`nombre` as 'nombre', `visita`.`direccion` as 'direccion', `visita`.`latitud` as 'latitud', `visita`.`longitud` as 'longitud'",
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'visita' => "`visita` ",
			'firmas' => "`firmas` LEFT JOIN `visita` as visita1 ON `visita1`.`id`=`firmas`.`id` ",
		];
